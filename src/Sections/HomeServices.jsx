...
const HomeServices = () => {
  ...
  return (
    <>
      <section
        id="testimonials"
        className="w-full lg:px-20 py-20 px-10 flex flex-col justify-center items-center gap-6">
        <h1
          data-aos="slide-up"
        ...
        <div
          id="client-grid"
          className="grid lg:grid-cols-3 grid-cols-1 gap-14 mt-10">
          {clients.map((item, index) => (
            <div
              data-aos="zoom-in"
              key={index}
              className="border-t-8 border-yellow-500 p-4 flex flex-col justify-center items-start gap-4">
              <FaQuoteLeft className="text-yellow-500 w-[38px] h-[38px]" />
              <h1 className="text-2xl text-black font-semibold">
                {item.title}
              </h1>
              <p className="text-gray-700">{item.para}</p>
              <button className="underline text-lg font-semibold text-yellow-500">
                {item.label}
              </button>
            </div>
          ))}
